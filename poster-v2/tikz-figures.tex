% ============================================================
% CWI 2026 poster visuals - TikZ source (FIXED 2026-08-16, rev2)
...
%  F4 (red-team S-1): Figure 2 node names (E1-\a etc.) were literal inside
%      \foreach (TikZ names don't expand) - misrendered. Rewritten to
%      coordinate-only drawing: every edge now targets computed polar
%      coordinates, so no node-name expansion is needed at all.
% ============================================================
...
\begin{tikzpicture}[scale=1.2, every node/.style={transform shape}]
  ...
  % Tier 1 Environment Qubits (Inner Ring) -- coordinate-only edges (F4)
  \foreach \a in {0, 120, 240} {
    \node[circle, draw=black, fill=red!70, inner sep=2pt] at (\a:0.9cm) {};
    \draw[thick, gray!70] (0,0) -- (\a:0.9cm);
  }
  ...
  \foreach \a in {0, 120, 240} {
    \foreach \b in {-30, 30} {
      ...
      \node[circle, draw=black, fill=red!50, inner sep=1.8pt] at (\ang:1.8cm) {};
      \draw[thick, gray!50] (\a:0.9cm) -- (\ang:1.8cm);
    }
  }
  ...
  \foreach \a in {0, 120, 240} {
    \foreach \b in {-30, 30} {
      \foreach \c in {-15, 15} {
        \pgfmathsetmacro{\ang}{\a + \b + \c}
        \pgfmathsetmacro{\parent}{\a + \b}
        \node[circle, draw=black, fill=red!30, inner sep=1.5pt] at (\ang:2.8cm) {};
        \draw[thick, gray!30] (\parent:1.8cm) -- (\ang:2.8cm);
      }
    }
  }
